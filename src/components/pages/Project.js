            ...
            <div className={styles.project_details}>
                <Container customClass="column">
                    ...
                    <div className={styles.service_form_container}>
                            ...
                            <div className={styles.project_info}>
                                {showServiceForm && <ServiceForm handleSubmit={createService} btnText="Adicionar tarefa" projectData={project} />}
                            </div>
                    </div>
                    <h2>Lista de tarefas</h2>
                    <Container customClass="start">
                        {services.length > 0 && services.map((service) => (
                            <ServiceCard id={service.id} name={service.name} description={service.description} key={service.id} hendleRemove={removeService} />
                        ))}
                        {services.length === 0 && <p>Não há tarefas na lista.</p>}
                    </Container>
                </Container>
            </div>
        ): (
